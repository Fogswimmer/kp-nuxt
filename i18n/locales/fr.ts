export default {
    welcome: 'Bienvenue',
    language: 'langue',
    contents: 'Contenu',
    logo_description: 'Centre des cinéphiles',
    page_descriptions: {
        home: 'Les meilleurs films et acteurs de tous les temps',
        films_list: 'Les meilleurs films de tous les temps',
        persons_list:
            'Les meilleurs acteurs, directeurs, producteurs, compositeurs, auteurs de tous les temps',
    },
    general: {
        no_network: 'Veuillez vérifier votre connexion réseau',
        show_more: 'Voir plus',
        hide: 'Cacher',
        max_file_size: 'Taille maximale des fichiers - { size }',
        max_file_size_error: 'Taille maximale des fichiers atteinte - { size }',
        youtube_link: 'Lien youtube',
        img_replacement_warning: "L'image actuelle sera remplacée. Continuer?",
        img_delete_warning: "L'image actuelle sera supprimée. Continuer?",
        img_broken: 'Image corrompue',
        about: 'A propos du projet',
        see_all: 'Voir tout',
        created_at: 'Créé',
        updated_at: 'Mis à jour',
        published_by: 'Publié par',
        total: 'Total: {count}',
        year: 'Année',
        years: 'Années',
        years_old: 'ans',
        empty: 'Vide',
        no_cover: 'Aucune couverture',
        no_avatar: 'Aucun avatar',
        data_transfer: 'Transfert de données',
        preview: 'Aperçu',
        items_per_page: 'Articles par page',
        ordering: 'Ordre',
        search: 'Rechercher',
        page: 'Page',
        sort: 'Trier par',
        of: 'de',
        first: 'Premier',
        last: 'Dernier',
        next: 'Suivant',
        prev: 'Précedent',
        loading: 'Chargement',
        no_data: 'Aucune données',
        no_results: 'Aucun resultat',
        no_data_found: 'Aucune données trouvées',
        no_results_found: 'Aucun resultat trouvé',
        no_img: 'Aucune image',
        uploaded: 'Fichier téléchargé',
        available_for_upload: 'Disponible pour le telechargement',
        img: 'Image',
        img_error: "Erreur chargement de l'image",
        uploaded_img: 'Image telechargée',
        drag_and_drop: "Déposez vos fichiers d'images ici",
        browse_files: 'Parcourir les fichiers',
        or: 'ou',
        selected: 'Selectionnées',
        for_upload: 'pour téléchargement',
        for_removal: 'pour suppression',
        max_files_error: 'Le nombre maximum de fichiers est atteint',
        file_replacement_warning:
            'Le fichier actuel sera remplace par le nouveau. Continuer?',
        entity_delete_warning:
            'Vous vous appretez de supprimer cette entité. Tous les fichiers ainsi que les données liées seront supprimées. Continuer?',
    },
    search: {
        title: 'Recherche',
        request_result: '{count} résultats trouvés',
        no_result: 'Aucun resultat trouvé',
    },
    empty_states: {
        title: 'Aucune données trouvées',
        subtitle: 'Aucune données trouvées',
        films: 'Aucuns films trouvés',
        actors: 'Aucuns acteurs trouvés',
        directors: 'Aucuns directeurs trouvés',
        persons: 'Aucunes personnes trouvées',
        countries: 'Aucunes pays trouvés',
        filmography: 'Aucune filmographie trouvée',
        bio: 'Aucune biographie trouvée',
        items: 'Rien trouvé',
        no_content: 'Aucun contenu',
        no_content_notice:
            "Avant vous pouvez ajouter un film, vous devez ajouter des acteurs et d'autres membres du film crew",
        actions: {
            navigate: 'Aller à la page',
            to_films: 'Aller à la liste des films',
            to_persons: 'Aller à la liste des personnes',
            add: 'Ajouter',
            add_persons: 'Ajouter des personnes',
            home: "Retourner a la page d'accueil",
        },
        no_persons_found:
            "Avant vous pouvez ajouter un film, vous devez ajouter des acteurs et d'autres membres du film crew",
        error_404: 'Page non trouvée',
        error_500: "Une erreur s'est produite",
    },
    auth: {
        admin: 'Admin',
        not_auth: 'Connectez-vous pour utiliser les actions',
        login_to_continue: 'Connectez-vous pour continuer',
        error: 'Erreur',
        invalid_credentials: 'Identifiants invalides',
        check_credentials: 'Veuillez verifier vos identifiants',
        forgot_password: 'Mot de passe oublie?',
        remember_me: 'Se souvenir de moi',
        sign_in: 'Connexion',
        sign_up: 'Inscription',
        sign_out: 'Déconnexion',
        password: 'Mot de passe',
        email: 'Email',
        login: 'Login',
        password_confirm: 'Confirmer le mot de passe',
        password_placeholder: 'Entrer le mot de passe',
        email_placeholder: 'Entrer votre email',
        register: "S'inscrire",
        continue_as_guest: 'Continuer en tant que visiteur',
        login_placeholder: 'Entrer votre login',
        about: 'A propos',
        logout_confirm: 'Voulez-vous vraiment vous déconnecter?',
        profile_photo: 'Photo de profil',
        display_name: "Nom d'affichage",
        password_reset: 'Réinitialiser le mot de passe',
        password_reset_title: 'Mot de passe oublié? Réinitialiser',
        password_reset_text:
            'Veuillez entrer votre adresse email pour recevoir un lien de réinitialisation de mot de passe.',
        password_reset_email: 'Email',
        password_reset_email_placeholder: 'Entrer votre email',
        password_reset_submit: 'Envoyer le lien de réinitialisation',
        new_password: 'Nouveau mot de passe',
        stepper: {
            first: 'Envoyer un lien de réinitialisation de mot de passe',
            second: 'Entrer le code envoyé par email',
            third: 'Entrer un nouveau mot de passe',
        },
        password_reset_messages: {
            success:
                'Le code de réinitialisation de mot de passe a été envoyé par email.',
            error: "Une erreur s'est produite lors de l'envoi du code de réinitialisation de mot de passe.",
            invalid_token: 'Le token est invalide',
            reset_success:
                'Votre mot de passe a été réinitialisé. Vous pouvez fermer cette page',
        },
    },
    nav: {
        title: 'Navigation',
        home: 'Accueil',
        settings: 'Paramètres',
        films: 'Films',
        menu: 'Menu',
        films_list: 'Films',
        persons_list: 'Personnes',
        films_add: 'Ajouter un film',
        persons: 'Personnes',
        persons_add: 'Ajouter une personne',
        actors: 'Acteurs',
        all: 'Liste',
        directors: 'Réalisateurs',
        edit: 'Modifier',
        language: 'Langue',
        account: 'Compte',
        pagination: {
            page: 'Page',
            of: 'de',
            total: 'Total',
            first: 'Premier',
            last: 'Dernier',
            next: 'Suivante',
            prev: 'Précedente',
        },
    },
    dialogs: {
        confirm: {
            title: 'Etes-vous certain?',
            yes: 'Oui',
            complete: 'Confirmer',
            cancel: 'Annuler',
            delete: 'Supprimer',
        },
        types: {
            error: 'Erreur',
            success: 'Succès',
            warning: 'Avertissement',
            info: 'Info',
        },
    },
    profile: {
        title: 'Profil',
        name: 'Nom',
        email: 'Email',
        password: 'Mot de passe',
        save: 'Enregistrer',
        logout: 'Se déconnecter',
        age: 'Age',
        last_login: 'Dernière connexion: {time}',
        about: 'A propos',
        delete_account: 'Supprimer le profil',
    },
    menu: {
        title: 'Menu',
        toggle_theme: 'Activer le mode sombre',
        theme: {
            title: 'Thème',
            light: 'Lumineux',
            dark: 'Sombre',
            system: 'Système',
        },
        settings: 'Paramètres',
    },
    actions: {
        title: 'Actions',
        stop_edit: 'Arrêter modification',
        close_window: 'Fermer la fenêtre',
        read_all: 'Lire tout',
        login_required: 'Autorisation requise',
        choose_cover: 'Choisir une couverture',
        choose_poster: 'Choisir un affiche',
        reload: 'Rafraîchir',
        view: 'Voir',
        submit: 'Soumettre',
        add: 'Ajouter',
        upload: 'Télécharger',
        edit: 'Modifier',
        editing: 'Mdification',
        back: 'Retour',
        remove: 'Supprimer',
        cancel: 'Annuler',
        save: 'Enregistrer',
        search: 'Recherche',
        login: 'Connexion',
        confirm: 'Confirmer',
        register: 'Enregistrement',
        logout: 'Déconnexion',
        reset: 'Réinitialiser',
        clear: 'Effacer',
        toggle_language: 'Changer la langue',
        add_film: 'Nouveau Film',
        edit_film: 'Modifier Film',
        remove_film: 'Supprimer Film',
        add_actor: 'Ajouter Acteur',
        edit_actor: 'Modifier Acteur',
        edit_person: 'Modifier Personne',
        remove_actor: 'Supprimer Acteur',
        add_director: 'Ajouter Réalisateur',
        edit_director: 'Modifier Réalisateur',
        remove_director: 'Supprimer Réalisateur',
        add_film_actor: 'Ajouter Acteur au Film',
        remove_film_actor: 'Supprimer Acteur du Film',
        pick_file: 'Choisir un fichier',
        skip: 'Passer',
        finish: 'Finir',
        close: 'Fermer',
        add_img: 'Ajouter images',
        download: 'Télécharger',
        share: 'Partager',
        delete_selected: 'Supprimer les sélections',
        add_more: 'Ajouter plus',
        select_for_upload: 'Choisir pour télécharger',
        expand: 'Expandir',
        collapse: 'Replier',
        to_page: 'Aller à la page',
        sort: 'Trier',
        limit: 'Limite',
        order: 'Order',
        filters: 'Filtres',
        asc: 'Ascendant',
        desc: 'Descendant',
        edit_cover: 'Modifier la couverture',
        edit_poster: "Modifier l'affiche",
        delete_cover: 'Supprimer la couverture',
        edit_avatar: "Modifier l'avatar",
        set_avatar: 'Mettre comme avatar',
        set_cover: 'Mettre comme couverture',
        set_poster: 'Mettre comme affiche',
        upload_trailer: 'Télécharger le trailer',
        delete_trailer: 'Supprimer le trailer',
        upload_gallery: 'Télécharger la galerie',
        link_trailer: 'Lien vers le trailer',
        delete_assessment_warning:
            'Etes-vous certain de vouloir supprimer cette évaluation?',
        comments_show: 'Afficher les commentaires',
        comments_hide: 'Cacher les commentaires',
    },
    state: {
        all: 'Toutes',
        active: 'Actives',
        inactive: 'Inactives',
        loading: 'Chargement',
    },
    toast: {
        success: 'Succès',
        error: 'Erreur',
        warning: 'Avertissement',
        info: 'Info',
        link_copied: 'Lien copié',
        messages: {
            success: {
                add: 'Ajouté avec succès',
                edit: 'Modifié avec succès',
                remove: 'Supprimé avec succès',
                files_added: 'Fichiers bien ajoutés',
                files_removed: 'Fichiers bien supprimés',
            },
            error: {
                add: 'Erreur ajoutant',
                edit: 'Erreur modifiant',
                remove: 'Erreur supprimant',
                connection: 'Connexion perdue',
            },
            server_errors: {
                400: 'La syntaxe de la requête est erronée',
                401: 'Identifiant ou mot de passe incorrect',
                404: 'Ressource non trouvée',
                500: "Erreur interne du serveur. Contactez l'administrateur ou essayez plus tard",
                503: 'Service temporairement indisponible ou en maintenance',
                403: "Le serveur a compris la requête, mais refuse de l'exécuter",
                405: 'Méthode de requête non autorisée',
                406: "La ressource demandée n'est pas disponible dans un format qui respecterait les en-têtes « Accept » de la requête",
                408: 'Temps d’attente d’une requête du client, écoulé côté serveur',
                409: "La requête ne peut être traitée à la suite d'un conflit avec l'état actuel du serveur",
                422: 'Unprocessable Entity',
                502: 'Bad gateway',
            },
        },
    },
    pages: {
        network_error: 'Erreur de connexion',
        general_info: 'Informations générales',
        detailed_info: 'Informations détallées',
        gallery: 'Galerie',
        home: {
            title: 'Accueil',
            newest: 'Les plus nouveaux',
            top: 'Les mieux notées {count} films',
            popular_persons: 'Personnes populaires',
            popular_actors: 'Acteurs populaires',
            buy_tickets: 'Acheter des billets',
            watch_trailer: 'Voir le trailer',
            watch_now: 'Regarder maintenant',
            watch_later: 'Regarder plus tard',
            actors: 'Acteurs',
            directors: 'Directeurs',
            language: 'Langue',
        },
        films: {
            title: 'Films',
            trailer: 'Trailer',
            poster: 'Poster',
            similar_genres: "Films similaires",
            related_persons: 'Personnes liées',
            country: 'Pays',
            budget: 'Budget',
            fees: 'Frais',
            edit_description: 'Modifier la description',
            edit_gallery: 'Modifier la galerie',
            list: 'Liste des films',
            details: 'Details',
            details_film: 'Details du film',
            back_to_list: 'Retourner vers la liste',
            name: 'Nom',
            release_year: 'Année de sortie',
            director: 'Réalisateur',
            actors: 'Acteurs',
            starring: 'Acteurs principaux',
            description: 'Description',
            duration: 'Durée',
            genre: 'Genre',
            no_preview: 'Aucun poster',
            gallery: 'Galerie',
            no_gallery: 'Galerie vide',
            add: 'Ajouter Film',
            edit: 'Modifier Film',
            remove: 'Supprimer Film',
            rating: 'Evaluation',
            assessments: '{count} avis',
            no_assessments: 'Aucun avis',
            your_assessment: 'Votre avis',
            comment: 'Commentaire',
            comments: 'Commentaires',
            last_comments: 'Derniers commentaires',
            no_comments: 'Aucun commentaire pour le moment',
            confirm_assessment: "Confirmer l'avis",
            producer: 'Producteur',
            writer: 'Auteur',
            age: "Restriction d'âge",
            no_actors_data: 'Aucune information sur les acteurs',
            assess: 'Evaluer',
            team: 'Equipe',
            gallery_banner:
                'Choississez les images à effacer ou ajouter de nouvelles si disponible',
            trailer_error: "Le trailer n'a pas pu charger",
        },
        persons: {
            related_films: 'Films liés',
            title: 'Personnes',
            details: 'Details de la personne',
            list: 'Liste des personnes',
            firstname: 'Prénom',
            lastname: 'Nom',
            birthday: 'Anniversaire',
            specialties: 'Specialités',
            featuredInFilms: 'Acté aux films',
            directed_films: 'Dirigé',
            written_films: 'Scénariste',
            produced_films: 'Producteur',
            composed_films: 'Compositeur',
            no_films: 'Aucuns films',
            no_specialties: 'Aucunes specialités',
            photos: 'Galerie',
            no_photos: 'Aucune photo',
            bio: 'Biographie',
            filmography: 'Filmographie',
            role: 'Rôle',
            edit_bio: 'Modifier la biographie',
        },
        profile: {
            title: 'Mon profil',
            edit: 'Modifier le profil',
            change_cover: 'Changer la couverture',
            upload_cover: 'Charger une couverture',
            settings: 'Paramètres',
            logout: 'Déconnexion',
            personal_info: 'Informations personnelles',
            avatar: 'Avatar',
            change_avatar: "Modifier l'avatar",
            upload_avatar: 'Charger un avatar',
            avatar_replace_warning: "L'avatar actuel sera remplacé. Continuer?",
            last_login: 'Dernière connexion: {time}',
        },
        about: {
            title: 'A propos',
            subtitle: 'Informations techniques',
            stack: 'Stack technique',
            scheme: 'Scheme des conteneurs',
            code: 'Code',
            table_headers: {
                frontend: 'Frontend',
                backend: 'Backend',
                language: 'Langue de programmation',
                main_framework: 'Framework principal',
                additional_framework: 'Framework UI',
                important_packages: 'Packages indispensables',
                internationalization: 'Internationalisation',
            },
            dev_prod: {
                title: 'Stack DevOps',
                table_headers: {
                    os: "Systèmes d'exploitation",
                    version_control: 'Contrôle de version',
                    pipeline: 'Pipeline',
                    containerisation: 'Conteneurisation',
                    scripts: 'Scripts',
                    message_brocker: 'Brocker messages',
                    cache_db: 'Db mise en cache',
                },
            },
            author: {
                title: 'Auteur',
                first_section:
                    "Ce projet démontre mes compétences actuelles en développement web. Outre la pile utilisée, j'ai de l'expérience avec",
                second_section: 'Directions de croissance',
                subtitle: 'Fullstack developer',
                name: 'André Diakov, fullstack developer',
                stack: 'Mon stack',
                about: 'A propos de moi',
                bio: '',
            },
        },
    },
    forms: {
        rules: {
            max_chars: 'Maximum de caractères - {max}',
            min_chars: 'Minimum de caractères - {min}',
            min_year: 'Année minimale - {year}',
            max_year: 'Année maximale - {year}',
            type_num: 'Entrez un nombre',
            required: 'Requis',
            min_birthday: 'Date de naissance minimale - {date}',
            max_birthday: 'Date de naissance maximale - {date}',
            only_latin_chars: 'Seuls les caractères latins sont autorisés',
            invalid_url: 'Entrez une url valide',
            invalid_email: 'Entrez une adresse email valide',
            only_letters: 'Seuls les lettres sont autorisées',
            invalid_password: 'Mot de passe invalide',
            password_requirement:
                'Le mot de passe doit comporter au moins 8 caractères et contenir au moins une lettre et un chiffre',
        },
        film: {
            slogan: 'Slogan',
            trailer: 'Trailer',
            add: 'Ajouter Film',
            edit: 'Modifier Film',
            name: 'Nom',
            name_international: 'Nom (International)',
            release_year: 'Année de sortie',
            director: 'Réalisateur',
            actors: 'Acteurs',
            description: 'Description',
            duration: 'Durée',
            genres: 'Genres',
            new_preview: 'Nouveau poster',
            gallery: 'Galerie',
            new_gallery: 'Nouvelle galerie',
            producer: 'Producteur',
            composer: 'Compositeur',
            writer: 'Auteur',
            age: "Restriction d'âge",
            update_poster: 'Mettre à jour le poster',
            update_gallery: 'Mettre à jour la galerie',
            gallery_actual: 'Galerie actuelle',
            gallery_item_delete_confirm:
                'Voulez-vous vraiment supprimer ce fichier de la galerie?',
        },
        gallery_max_count: 'Le nombre de fichiers ne doit pas dépasser 6.',
        gallery_restrictions:
            'Formats acceptés: jpg, png, jpeg, webp. Taille maximale - 4 MB.',

        gallery_empty_warning:
            "Vous n'avez pas chargé de fichier pour la galerie, mais vous pouvez le faire plus tard. Continuer?",
    },
    person: {
        add: 'Ajouter personne',
        edit: 'Modifier personne',
        add_specialty: 'Ajouter specialité',
        firstname: 'Nom',
        lastname: 'Prénom',
        name_international: 'Nom (International)',
        gender: 'Genre',
        birthday: 'Anniversaire',
        age: 'Age',
        specialties: 'Specialités',
        actor: 'Acteur',
        director: 'Directeur',
        producer: 'Producteur',
        writer: 'Auteur',
        featuredInFilms: 'Acté aux films',
        directed_films: 'Réalisé les films',
        produced_films: 'Produit les films',
        composed_films: 'Composé la musique pour les films',
        written_films: 'Auteur des scénarios aux films',
        no_films: 'Aucuns films',
        photo: 'Photo',
        no_photo: 'Aucune photo',
        bio: 'Biographie',
        photo_restrictions:
            'Chargez un fichier dans le format jpg, png, jpeg. Taille maximale - 2 MB.',
        no_photo_warning:
            "Vous n'avez pas chargé de photo, mais vous pouvez le faire plus tard. Continuer?",
        gender_hint: 'Choisissez le genre',
        specialty_hint: 'Choisissez les specialités',
    },
    stepper: {
        general: 'Informations principales',
        gallery: 'Télécharger la galerie',
        cover: 'Télécharger la couverture',
        poster: 'Mettre comme le poster',
        avatar: 'Mettre une photo de profil',
        avatar_upload: 'Télécharger une photo de profil',
    },
    filters: {
        title: 'Filtres',
        records: 'Enregistrements',
        display: 'Afficher',
        order: {
            title: 'Ordre',
            asc: 'Ascendant',
            desc: 'Descendant',
        },
        sort: {
            title: 'Trier',
            name: 'Par titre',
            all: 'Tous',
            films: {
                year_of_release: 'Année de sortie',
                rating: 'Note',
                duration: 'Durée',
            },
            persons: {
                name: 'Nom',
                firstname: 'Prénom',
                lastname: 'Nom',
                birthday: 'Anniversaire',
                age: 'Age',
                specialties: {
                    title: 'Specialités',
                    actors: 'Acteurs',
                    directors: 'Directeurs',
                    producers: 'Producteurs',
                    writers: 'Auteurs',
                    composers: 'Compositeurs',
                },
            },
        },
    },
}
